...
LLM Evaluation: Iteratively send each essay-feedback pair to the LLM (GPT here) with a strict rubric.

Save Results: Store the quantitative scores and qualitative justifications back to Google Drive for analysis.

***1. Environment Setup & Configuration***

In this section, we install necessary dependencies, load API keys from the environment, and define the core "Judge Persona."

System Role: Defines the LLM as an expert professor in educational assessment.

Evaluation Rubric: Specifies the 5-point scale criteria for Tone, Level of detail, Grammar, Stucture, Content.
"""
...
def get_llm_evaluation(essay: str, feedback: str, client: OpenAI) -> Dict[str, Any] | None:
    ...
    try:
        ...
        # Extract text (new Responses API output)
        output_text = response.output_text
...
def main():
    ...
    OUTPUT_PKL_PATH = '/content/drive/MyDrive/Practicum the Feedback Desk/GPT_judges_evaluation_results.pkl'
    ...
    try:
        ...
        print(f"✅ Data merged! Total unique essays to process: {len(merged_data)}")

        print(f"🔍 Debug: Merged Columns: {merged_data.columns.tolist()}")
...
# 2. Loop processing (using tqdm to display progress bar)
    ...
    for index, row in tqdm(merged_data.iterrows(), total=len(merged_data)):
        ...
        # Handling potential suffix combinations
            ...
            print(f"⚠️ Column missing for ID {current_id}: Check column names!")
            current_essay = ""
        ...
        if pd.isna(current_essay) or len(str(current_essay)) < 10:
            ...
            print(f"⚠️ Skip ID {current_id}: Content empty.")
            continue
...
# 3. Save results
    ...
    print(f"\n💾 Saving {len(final_df)} unique results to Drive...")
